...
public class Tree<T> {
    ...
    public List<Node<T>> getNodeListByLevel(short nodeLevel){
    	List<Node<T>> nodeList = new ArrayList<Node<T>>(); 
    	insertNodeListByNodeLevel(rootElement,nodeList,nodeLevel);
    	return nodeList;
    }
    
    public void insertNodeListByNodeLevel(Node<T> parentNode,List<Node<T>> nodeList,short nodeLevel ){
    	if(parentNode.getNodeLevel()<nodeLevel){
    		if(parentNode.getChildren().size()!=0){
    			for(Node<T> childNode:parentNode.getChildren()){
    			insertNodeListByNodeLevel(childNode, nodeList, nodeLevel);
    			}
    		}
    	}else if(parentNode.getNodeLevel()==nodeLevel){
